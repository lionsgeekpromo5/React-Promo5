...
function ProductDetails() {
    ...
    const productIndex = database.findIndex(e => e.id == id)

    const addToCart = () => {
        const newProducts = [...database]
        newProducts[productIndex].stock = newProducts[productIndex].stock - 1
        setDatabase(newProducts)
        console.log(database[productIndex].stock)
  ...
  return (
    <div>
      <Navbar />
      <div  className=' h-[85vh] grid grid-cols-[30%_1fr] p-10 gap-x-2'>
        <div className='border-2 border-orange-400 rounded-md'>
            <img src={database[productIndex].images} alt="" />
        </div>
        <div className='px-10 flex flex-col gap-y-2'>
            <h1 className='text-3xl font-medium'>{database[productIndex].title}</h1>
            <p className=' text-[25px] font-thin'>{database[productIndex].description}</p>
            <span className='flex items-center gap-x-2'><FaStar color='yellow' size={20}/> {database[productIndex].rating} ({database[productIndex].reviews.length} Reviews)</span>
            <div className='flex gap-x-10 items-center '>
                <span  className='text-3xl font-bold '>${database[productIndex].price}</span>
                <span className='text-xl font-thin'>Stock: {database[productIndex].stock} </span>
            </div>
            <button className=' py-2 rounded-md bg-green-700 w-[300px] flex items-center justify-center gap-x-4 text-white' onClick={() => addToCart()}><FaShoppingCart size={20}/>Add To Cart</button>
        </div>
